...
// В функции setupEventListeners убедитесь, что обработчик добавлен правильно
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    await initGame();
    initTonConnect();
    setupEventListeners(); // Перенесли вызов setupEventListeners после initGame
    updateUI();
...
function setupEventListeners() {
    ...
    if (buyAutoclickerBtn) buyAutoclickerBtn.addEventListener('click', buyAutoClicker);
    if (collectBtn) collectBtn.addEventListener('click', collectOfflineCoins);
...
// Клик по монете (добавляем проверки)
function clickCoin() {
    if (!userId) return;
